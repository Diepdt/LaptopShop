...
import { getCartPage, getClientProductPage, postAddProductToCart } from '../controllers/client/product.controllers';
...
const webRoutes = (app: Express) => { // khai bao 1 ham va dat ten: webRoutes
    ...
    router.get("/user/authentication", getAuthentication);
    router.get("/user/register", getUserRegisterPage);
    router.post("/user/register", postRegister);
    router.get("/user/login", isLogin, getUserLoginPage);
    router.post("/user/login", passport.authenticate('local', {
        successRedirect: "/user/authentication",
